...
def plot_clusters(data, kmeans, x_column, y_column, title):
    ...
    for cluster in range(kmeans.n_clusters):
        ...
        plt.scatter(
            cluster_data[x_column], cluster_data[y_column], label=f'Cluster {cluster}')
...
def curve_fit_and_predict(data, country_name, x_column, y_column):
    ...
    plt.plot(prediction_years, predicted_values, 'x-',
             label=f'{country_name} Predicted (1990-2030)')
    plt.xlabel('Year')
    plt.ylabel(f'{y_column}')
    plt.title(
        f'{y_column} for {country_name} with Extended Curve Fit (1990-2030)')
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(x_data, y_data, 'o-', label=f'{country_name} Actual Data')
    plt.plot(prediction_years, curve_fit_function(
        prediction_years, *popt), 'x-', label=f'{country_name} Best Fit')
...
merged_data = pd.merge(co2_data, forest_data, on=[
